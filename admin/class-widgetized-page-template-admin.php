...
class Widgetized_Page_Template_Admin {
	...
	public function register_page_widget_area() {
		...
		$args = array(
			...
			'meta_value'     => 'page_widgetized.php', // phpcs:ignore WordPress.DB.SlowDBQuery
			'post_status'    => 'any',
			'orderby'        => 'name',
			'order'          => 'ASC',
		...
		foreach ( $the_query->posts as $query ) {
			...
			if ( $page_title && function_exists( 'genesis_register_sidebar' ) ) {
				genesis_register_sidebar(
					array(
						'id'          => $page_id,
						'name'        => esc_html__( 'Page', 'widgetized-page-template' ) . ' - ' . $widget_title,
						/* translators: The page title. */
						'description' => sprintf( esc_html__( 'This is the "%s" page widget area.', 'widgetized-page-template' ), $page_title ),
					)
				);
			}
		}
	...
	public function add_admin_notice() {
		...
		$url_text                    = esc_html__( 'Widgets', 'widgetized-page-template' );
		$button_text                 = esc_html__( 'Cick Here to Edit Live', 'widgetized-page-template' ); ?>

		<div class="notice notice-warning inline">
			<p>
				<?php
				echo sprintf(
					/* translators: Widgets' admin page link. */
					esc_html__( 'You are currently editing a Widgetized Page template. To edit the contents of this page, you can go to the %s page and edit its corresponding widget area, or click below to start editing live.', 'widgetized-page-template' ),
					'<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">' . esc_html( $url_text ) . '</a>'
				);
				?>
			</p>

			<p>
				<a class="button button-secondary" href="<?php echo esc_url( $section_link ); ?>"><?php echo esc_html( $button_text ); ?></a>
			</p>
		</div>
		<?php
